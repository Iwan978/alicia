...
function flipFn(horizontal, vertical, cb) {
  ...
  if (typeof cb === "function" && !horizontal && !vertical) {
    return throwError.call(
      this,
      "At least one of horizontal or vertical must be true",
      cb
    );
  }

  if (typeof this.bitmap.width !== "number" || this.bitmap.width < 0) {
    return throwError.call(
      this,
      "this.bitmap.width must be a non-negative number",
      cb
    );
  }

  if (typeof this.bitmap.height !== "number" || this.bitmap.height < 0) {
    return throwError.call(
      this,
      "this.bitmap.height must be a non-negative number",
      cb
    );
  }

  if (typeof this.scanQuiet !== "function") {
    return throwError.call(
      this,
      "this.scanQuiet is not a function",
      cb
    );
  }
