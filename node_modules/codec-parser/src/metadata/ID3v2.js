...
import { length as lengthConstant, version, readRawData } from "../constants.js";
...
export default class ID3v2 {
  static *getID3v2Header(codecParser, headerCache, readOffset) {
    if (typeof readOffset !== "number" || readOffset < 0) {
      throw new Error("readOffset must be a non-negative number");
    }

    const headerLength = 10;
    const header = {};

    if (!codecParser || !codecParser.readRawData) {
      throw new Error("codecParser must have a readRawData method");
    }
    ...
    if (dataLength < headerLength || dataLength > 0xFFFFFF) {
      throw new Error("Invalid data length");
    }

    header[lengthConstant] = headerLength + dataLength;
  ...
  constructor(header) {
    ...
    this[lengthConstant] = header[lengthConstant];
  }
}
